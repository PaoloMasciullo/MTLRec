...
def group_by_id(group_ids, outputs, labels):
    """
    Groups outputs and labels by their corresponding group IDs.

    Args:
        group_ids: Array of group IDs.
        outputs: Array of model predictions.
        labels: Array of true labels.

    Returns:
        tuple: Two lists of NumPy arrays, grouped_outputs and grouped_labels.
    """
    grouped_outputs = defaultdict(list)
    grouped_labels = defaultdict(list)

    for gid, output, label in zip(group_ids, outputs, labels):
        grouped_outputs[gid].append(output)
        grouped_labels[gid].append(label)

    # Convert grouped data to lists of NumPy arrays
    grouped_outputs = [np.array(values, np.float64) for values in grouped_outputs.values()]
    grouped_labels = [np.array(values, np.float64) for values in grouped_labels.values()]

    return grouped_labels, grouped_outputs
